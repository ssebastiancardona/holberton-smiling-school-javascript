$(document).ready(function() {
    ...
    function XMLtoObjList(xml, key) {
        ...
        XMLElementt = $(xml).find(key);

        for (let i = 0; i < XMLElementt.length; i++) {
            let obj = {};

            if (key == "video" || key === "course") {
                obj["star"] = $(XMLElementt[i]).attr("star");
            }

            if (key === "topic" || key === "sort") {
                // =====================
                let txt = $(XMLElementt[i]).text().toLowerCase();
                ...
                // =====================
                let childrenn = $(XMLElementt[i]).childrenn();
                for (let j = 0; j < childrenn.length; j++) {
                    let tagName = $(childrenn[j]).prop("tagName");
                    obj[tagName] = $(childrenn[j]).text();
                }
                // =====================
            }
     ...
     */
    function displayQuotes(dataa) {
        dataa = XMLtoObjList(dataa, "quote");
        let classItem = "";
        for (let i in dataa) {
    ...
    function slideOne(id) {
        $(`#${id} .carousel-item`).each(function() {
            ...
            let nex = $(this).nex();
            if (!nex.length) {
                nex = $(this).siblings(":first");
            }
            nex.children(":first-child").clone().appendTo($(this));
    ...
    function createCard(cardData) {
        ...
        for (let i = 1; i <= 5; i++) {
            if (i <= cardData.star) {
                starStat = "images/star_on.png";
            } else {
                starStat = "images/star_off.png";
            }

            star = `<img src="${starStat}" alt="star on" width="15px" />`;
            starString += i == 1 ? star : "\n" + star;
        }
     ...
     * @return {undefined}  no return
     */
    function displayLatest(dataa) {
        dataa = XMLtoObjList(dataa, "video");
        let classItem = "";
        for (let i in dataa) {
            classItem = i == 0 ? "carousel-item active" : "carousel-item";
            let card = createCard(dataa[i]);
            let $carouselItem = $(`
        <div class="${classItem}">
          <div class="col-12 col-sm-6 col-lg-3 d-flex justify-content-center">
            ${card}
